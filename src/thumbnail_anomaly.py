...
VIDEO_NAME = "Vandalism028_x264"  # Change this for different videos
...
if gt_segments:
    gt_width = 3
    gt_plot_start = max(0, peak_idx - gt_width//2)
    gt_plot_end = min(len(scores), peak_idx + gt_width)
    
    ax.axvspan(gt_plot_start, gt_plot_end, color='lightblue', alpha=0.5, label='GT Anomaly')
    gt_plotted = True
...
# Peak image - SAME SIZE AS VALLEY IMAGE, SAFE Y POSITION
if peak_frame is not None:
    # Put peak image to the LEFT of the peak, SAME SIZE as valley image
    peak_img_left = peak_idx - 3.5   # Same width as valley (3 segments)
    peak_img_right = peak_idx - 0.5
    
    # Use SAFE Y position instead of peak_text_y (which might be too high)
    peak_img_bottom = 0.50   # Safe Y position within 0-0.9 range
    peak_img_top = 0.66      # Same height difference as valley (0.16 units)
    
    print(f"ADDING PEAK IMAGE (SAME SIZE, SAFE Y): [{peak_img_left}, {peak_img_right}, {peak_img_bottom}, {peak_img_top}]")
    ...
    print("PEAK IMAGE ADDED - SAME SIZE AS VALLEY, SAFE POSITION!")

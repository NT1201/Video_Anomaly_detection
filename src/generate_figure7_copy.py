...
# === Paths ===
# Detect if this is a normal or anomaly video for file paths only
...
# Check if there's actually a significant anomaly - MUCH STRICTER THRESHOLDS
score_range = peak_val - valley_val
mean_score = np.mean(scores)

# For a REAL anomaly, we need BOTH conditions:
# 1. High peak score AND significant variation
# 2. Only for actual anomaly videos (not normal videos)
is_real_anomaly_video = not is_normal_video  # Don't show peaks for normal videos at all
...
# GT region around peak - ONLY SHOW IF ANOMALY DETECTED
...
# Annotations - ONLY SHOW IF REAL ANOMALY DETECTED
...
if is_anomaly_detected:
    ...
    print("Showing Peak/Normal annotations (anomaly detected)")
    ...
    print("Images shown (anomaly detected)")
else:
    print("No significant anomaly detected - no Peak/Normal annotations or images shown")
...
if is_anomaly_detected:
    ...
    print("No significant anomaly detected - no Peak/Normal annotations shown")

# Add BOTH images in guaranteed visible locations
if valley_frame is not None:
    print("Adding valley frame...")
    ax.imshow(valley_frame, extent=[valley_idx + 0.5, valley_idx + 3.5, valley_text_y + 0.02, valley_text_y + 0.18], 
              aspect='auto', zorder=10)

if peak_frame is not None:
    print("Adding peak frame...")
    ax.imshow(peak_frame, extent=[peak_idx - 3.0, peak_idx - 0.5, peak_text_y + 0.02, peak_text_y + 0.18], 
              aspect='auto', zorder=10)

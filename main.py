# main.py ──────────────────────────────────────────────────────────────
# Multiple-instance learning for video anomaly detection + rich logging
...
def make_loaders(root, feat, batch, seg):
    """paired loaders for normal / anomaly – train & test"""
    ...
    g = torch.Generator().manual_seed(0)        # reproducible shuffle
    return (
...
def evaluate(model, ld_a, ld_n, device):
    """max-pool segment logits → 1 score / video"""
    model.eval()
    scores, labels = [], []
    with torch.no_grad():
        for clip in ld_a:                                  # anomaly videos
            s = model(clip.view(-1, clip.size(-1)).to(device))
            scores.append(torch.sigmoid(s).max().item()); labels.append(1)
        for clip in ld_n:                                  # normal videos
            s = model(clip.view(-1, clip.size(-1)).to(device))
...
def main():
    ...
    ap.add_argument("--save_scores",  action="store_true",
                    help="save *per-video* scores runs/scores_epXX.npy")
    ap.add_argument("--dump_curve",   action="store_true",
                    help="also dump 1 segment-curve per epoch (segcurve_epXX.npy)")
    ap.add_argument("--curve_vid",    type=int, default=0,
                    help="index of test video used for curve dump")
    args = ap.parse_args()
    ...
    for ep in range(1, args.epochs+1):
        ...
        # ▸ CSV row  – keep ONLY scalar items
        ...
        # ▸ optional per-video scores
        if args.save_scores:
            np.save(f"runs/scores_ep{ep:02d}.npy", y_score)

        # ▸ optional per-epoch segment curve (for Fig-8 evolution)
        if args.dump_curve:
            clip = ld_a_te.dataset[args.curve_vid]   # choose one anomaly video
            with torch.no_grad():
